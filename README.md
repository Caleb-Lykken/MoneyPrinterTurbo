...
This fork adds **unattended batch production and direct publishing** on top of
upstream MoneyPrinterTurbo. Upstream generates one video per run from one topic;
this fork takes a list of topics and runs the whole pipeline end to end.

**Batch generation** — `--subjects-file topics.txt` writes a script for every
topic into a resumable manifest, then renders videos one at a time. A failed
topic is recorded and the batch continues. Interrupt it at any point and rerun
with `--batch-manifest`: finished topics are skipped, failed ones are retried,
and nothing is rendered twice.

**Direct YouTube publishing** — uploads through your own Google Cloud project via
the YouTube Data API (`--publish`), rather than a third-party service. Titles,
descriptions and tags come from the LLM, and every upload declares
`containsSyntheticMedia`. Re-running a batch never re-uploads a video that
already has an id.

**Batch UI** — `sh batch-ui.sh` opens a separate Streamlit app (port 8601,
independent of the main WebUI) to start batches and watch them live: per-topic
status, assigned music and voice, scripts, logs, and finished videos. It also
shows how published videos are performing, searches your niche for what is
currently getting views, and can generate new topics from those patterns.

**Unattended automation** — `scripts/auto_daily.py` publishes any backlog first,
then researches the niche and renders new topics. Rendering and publishing can be
scheduled separately, so uploads are spread across the day instead of arriving in
one burst.

**Output quality defaults for English Shorts** — random US voices per topic,
subtitles positioned clear of the Shorts UI overlay, a bold Latin font, and your
own background music rotated across a batch. Upstream defaults are tuned for
Chinese content and read poorly with English scripts.

**Disk management** — `--delete-after-upload` removes local task files once
YouTube confirms an upload, which matters at roughly 65 MB per video.

Everything above is additive: no upstream behaviour changed, and every existing
command works exactly as before. See `test/services/test_batch*.py`,
`test_youtube*.py` for coverage of the new code.
...
#### ⑤ Batch Mode and YouTube Publishing 📺

Generate one video per topic from a list. Write the topics one per line in a text
...
Scripts for every topic are generated first and written to a batch manifest, then
videos are rendered one at a time. The manifest is saved after each step, so an
interrupted batch can be resumed with the same file — finished topics are skipped
and failed topics are retried:

```shell
uv run python cli.py --batch-manifest storage/batches/<batch-id>/manifest.json
```

A topic that fails is recorded and the batch continues with the next one. Use
`--stop-at script` to generate every script first and review or edit the manifest
before rendering.

The project's default voice is a Chinese one, which reads English scripts with a
heavy accent. For English content, pick a random US voice per topic:

```shell
uv run python cli.py --subjects-file topics.txt --random-voice en-US
```

Voices that require an Azure V2 key are excluded automatically, and the voice
chosen for each topic is recorded in the manifest so a resumed batch reuses it.

To upload each finished video to YouTube through your own Google Cloud project:

1. Enable **YouTube Data API v3** in the Google Cloud console.
2. Create an OAuth client of type **Desktop app** and save the JSON as
   `storage/youtube_client_secret.json`.
3. Install the optional dependencies: `uv sync --extra youtube`
4. Authorize once: `uv run python cli.py --youtube-auth`
5. Set `youtube_upload_enabled = true` in `config.toml`, or pass `--publish`.

```shell
uv run python cli.py --subjects-file topics.txt --publish
```

> **Note:** videos uploaded through the API by a project that has not passed
> Google's audit are forced to **private**, whatever `--youtube-privacy` requests.
> The command warns when this happens. Request an audit in the Google Cloud
> console to publish publicly through the API, or publish the videos manually
> from YouTube Studio in the meantime.

Long runs fill up the disk — roughly 65 MB per video in `storage/tasks/`. Add
`--delete-after-upload` to remove each task folder once YouTube confirms the
upload:
...
Deletion only ever happens after a real video id comes back, so a failed or
skipped upload always keeps the local files. It is ignored entirely when
publishing is off.

#### ⑥ Batch UI 🖥️

A separate browser UI for starting and watching batches, independent of the main
WebUI (both can run at once):

```shell
sh batch-ui.sh
```

It opens on http://127.0.0.1:8601 and lets you type a list of topics, pick a
music folder, and start a run. Batches run as their own process, so refreshing
the page or closing the tab never interrupts a render. The dashboard shows
per-topic status, the assigned background music track, generated scripts, the
live log, finished videos, and YouTube links.
